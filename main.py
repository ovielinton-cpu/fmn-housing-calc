...
async def build_ui(page: ft.Page):
    ...
    async def build_main_app():
        ...
        async def open_notification_sync_dialog(e):
            ...
            def make_pkg_checkbox(pkg, info):
                label = f"{pkg}  ({info.get('count', 0)}x)\n{info.get('sample', '')}"
                cb = ft.Checkbox(label=label, value=(pkg in allowed or not allowed))
                checkboxes[pkg] = cb
                return cb

            pkg_rows = [
                make_pkg_checkbox(pkg, info)
            ...
            async def sync_now(ev):
                ...
                await refresh_money_ui()
                page.pop_dialog()
                await open_notification_sync_dialog(ev)  # reopen fresh so newly-seen apps show up

            async def reset_allowlist(ev):
                _save_allowed_packages([])  # empty list = capture every app again
                page.pop_dialog()
                await open_notification_sync_dialog(ev)
                ...
                content=ft.Container(
                    content=ft.Column(
                        [
                            ...
                            ft.Container(content=ft.ListView(controls=pkg_rows, spacing=8), height=280),
                        ],
