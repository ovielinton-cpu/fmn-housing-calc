...
async def build_ui(page: ft.Page):
    ...
    async def build_main_app():
        ...
        async def backup_data(e):
            ...
            (page.open if hasattr(page, "open") else page.show_dialog)(ft.SnackBar(ft.Text("Backup saved! Keep this file somewhere safe." if file_path else "Backup cancelled.", color="white"), bgcolor=PURPLE_TEXT, duration=ft.Duration(seconds=3)))
            page.update()
        ...
        async def restore_data(e):
            ...
            if not files:
                (page.open if hasattr(page, "open") else page.show_dialog)(ft.SnackBar(ft.Text("Restore cancelled.", color="white"), bgcolor=PURPLE_TEXT, duration=ft.Duration(seconds=3)))
                page.update()
                return

            picked = files[0]
            if not getattr(picked, "bytes", None):
                (page.open if hasattr(page, "open") else page.show_dialog)(ft.SnackBar(ft.Text("Could not read that file. Please try again.", color="white"), bgcolor="#C62828", duration=ft.Duration(seconds=3)))
                page.update()
                return
            ...
            try:
                ...
                (page.open if hasattr(page, "open") else page.show_dialog)(ft.SnackBar(ft.Text("Backup restored! Your transactions are back.", color="white"), bgcolor="#2E8B57", duration=ft.Duration(seconds=3)))
                await refresh_money_ui()
                page.update()
            except Exception:
                (page.open if hasattr(page, "open") else page.show_dialog)(ft.SnackBar(ft.Text("That doesn't look like a valid backup file.", color="white"), bgcolor="#C62828", duration=ft.Duration(seconds=3)))
                page.update()
