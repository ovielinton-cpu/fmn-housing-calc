...
async def build_ui(page: ft.Page):
    ...
    async def build_main_app():
        ...
        async def backup_data(e):
            money_transactions = await load_transactions()
            ...
            backup_obj = {
                ...
                "profile_number": active_profile_state["value"],
            ...
            file_path = await pdf_file_picker.save_file(
                ...
                file_name=f"NairaFinanceHub_{active_name.replace(' ', '_')}_Backup_{datetime.date.today().isoformat()}.json",
                file_type=ft.FilePickerFileType.CUSTOM,
                allowed_extensions=["json"],
                src_bytes=backup_bytes,
            )
            page.show_dialog(ft.SnackBar(ft.Text(f"Backup saved for {active_name}! Keep this file somewhere safe." if file_path else "Backup cancelled.", color="white"), bgcolor=PURPLE_TEXT, duration=ft.Duration(seconds=3)))
            page.update()
        ...
        async def restore_data(e):
            ...
            try:
                ...
                active_name = profile_names_state.get(str(active_profile_state["value"]), f"Profile {active_profile_state['value']}")
                page.show_dialog(ft.SnackBar(ft.Text(f"Backup restored into {active_name}!", color="white"), bgcolor="#2E8B57", duration=ft.Duration(seconds=3)))
                await refresh_money_ui()
                page.update()
            except Exception:
        ...
        profile_label_text = ft.Text(
            ...
            color="#4B0082", weight=ft.FontWeight.BOLD, size=14,
        )
        ...
        profile_switcher_btn = ft.ElevatedButton(
            content=ft.Row(
                [ft.Icon(ft.Icons.SWITCH_ACCOUNT_ROUNDED, size=16, color=PURPLE_TEXT), profile_label_text, ft.Icon(ft.Icons.ARROW_DROP_DOWN, color=PURPLE_TEXT)],
