        ...
        def build_ajo_pdf_bytes(group):
            ...
            if order:
                ...
                if current_round <= len(order):
                    collector = sanitize_for_pdf(order[current_round - 1])
                    ...
                    paid_members = [m for m in order if payments.get(m, False)]
                    unpaid_members = [m for m in order if not payments.get(m, False)]
                    pdf.set_font("Helvetica", "B", 12)
        ...
        async def refresh_ajo_ui():
            ...
            if not active_group.get("order"):
                ...
                async def manual_order_dialog(e):
                    ...
                    if not active_group.get("contribution_amount", 0):
                        page.show_dialog(ft.SnackBar(ft.Text("Enter the contribution amount per person first.", color="white"), bgcolor="#C62828", duration=ft.Duration(seconds=3)))
                        page.update()
                        return
                ...
                async def reveal_manual_order(chosen_order, due_date):
                    ...
                    active_group["due_day"] = due_date.day
                    active_group["current_round"] = 1
                    active_group["payments"] = {"1": {m: False for m in chosen_order}}
                    active_group["history"] = []
                    active_group.pop("pending_due_date", None)
                    await save_ajo_groups(groups)
            ...
            async def edit_order_dialog(e):
                ...
                async def save_edit_order(e):
                    ...
                    active_group["payments"][str(current_round)] = {m: False for m in pending}
                    await save_ajo_groups(groups)
                    page.pop_dialog()
                    await refresh_ajo_ui()
            ...
            async def do_advance():
                ...
                if next_round <= len(order):
                    ...
                    page.show_dialog(ft.SnackBar(ft.Text(f"➡️ Next up: {next_collector} — contribute for their round!", color="white"), bgcolor="#8E24AA", duration=ft.Duration(seconds=4)))
                else:
                    page.show_dialog(ft.SnackBar(ft.Text("🎉 That was the last round — the cycle is complete!", color="white"), bgcolor="#FFD700", duration=ft.Duration(seconds=4)))
                page.update()
            ...
            ajo_content_area.content = ft.Container(
                content=ft.Column(
                    [
                        ...
                        ft.Divider(height=1, color="#4B0082"),
                        ft.Text(f"⏳ Unpaid ({len(order) - paid_count})", color="#C62828", weight=ft.FontWeight.BOLD, size=13),
                        ft.Container(
        ...
        async def import_ajo_group(e):
            ...
            try:
                ...
                if existing_idx is not None and fresh_groups[existing_idx].get("is_admin", True):
                    page.show_dialog(ft.SnackBar(ft.Text("You are the Admin of this group — import blocked to protect your copy.", color="white"), bgcolor="#C62828", duration=ft.Duration(seconds=4)))
                    page.update()
                    return
        ...
        async def open_ajo_switcher(e):
            ...
            async def export_ajo_pdf(e):
                ...
                try:
                    ...
                    page.show_dialog(ft.SnackBar(ft.Text(f"Export failed: {ex}", color="white"), bgcolor="#C62828", duration=ft.Duration(seconds=5)))
                    page.update()
