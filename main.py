...
async def build_ui(page: ft.Page):
    ...
    TUTORIAL_SLIDES = [
        ...
        },
        {
            ...
            "text": "Run your Ajo/esusu group digitally — support for multiple groups at once. Add members, set the contribution amount, then tap 'Draw Order' for a fair, animated random draw — no more paper and pen. Track whose turn it is, mark who has paid each round, and view full history once the cycle is complete.",
        },
    ]
    ...
    async def build_main_app():
        ...
        async def refresh_ajo_ui():
            ...
            if not active_group.get("order"):
                ...
                reveal_col = ft.Column([], spacing=6, horizontal_alignment=ft.CrossAxisAlignment.CENTER)
                ...
                async def draw_order(e):
                    ...
                    reveal_col.controls = [ft.Text("🎲 Drawing order...", color="#FFD700", weight=ft.FontWeight.BOLD, size=16)]
                    page.update()
                    await asyncio.sleep(0.6)
                    revealed = []
                    for i, name in enumerate(members):
                        c = ft.Container(
                            content=ft.Text(f"{i + 1}. {name}", color="white", weight=ft.FontWeight.BOLD, size=15),
                            scale=0,
                            animate_scale=ft.Animation(400, ft.AnimationCurve.BOUNCE_OUT),
                        )
                        revealed.append(c)
                        reveal_col.controls = [ft.Text("🎲 Drawing order...", color="#FFD700", weight=ft.FontWeight.BOLD, size=16)] + revealed
                        page.update()
        ...
        async def open_ajo_switcher(e):
            ...
            dlg = ft.AlertDialog(
                ...
                content=ft.Container(
                    content=ft.Column(
                        rows + [ft.Divider(height=1, color="#DDDDDD"), ft.TextButton("+ New Group", on_click=new_group_from_switcher, style=ft.ButtonStyle(color="#2E8B57"))],
