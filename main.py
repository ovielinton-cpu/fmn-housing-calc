            ...
            if not active_group.get("order"):
                ...
                for i, m in enumerate(active_group.get("members", [])):
                    ...
                    members_col.controls.append(
                        ft.Row(
                            [ft.Text(m, color="white", size=13), ft.IconButton(icon=ft.Icons.CLOSE, icon_color="#C62828", icon_size=16, on_click=make_remove_member(i))],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        )
                    )

                new_member_field = ft.TextField(hint_text="Member name", bgcolor="#FFFFFF", color=PURPLE_TEXT, text_style=ft.TextStyle(color=PURPLE_TEXT, weight=ft.FontWeight.BOLD), capitalization=ft.TextCapitalization.WORDS, dense=True, expand=True)
                ...
                contribution_field = ft.TextField(
                    ...
                    keyboard_type=ft.KeyboardType.NUMBER, bgcolor="#FFFFFF", color=PURPLE_TEXT,
                    text_style=ft.TextStyle(color=PURPLE_TEXT, weight=ft.FontWeight.BOLD),
                )
                ...
                async def run_the_draw(due_date):
                    ...
                    result_col = ft.Column([], spacing=16, horizontal_alignment=ft.CrossAxisAlignment.CENTER, scroll=ft.ScrollMode.AUTO)
                    dice_icon = ft.Icon(ft.Icons.CASINO_ROUNDED, size=80, color="#FFD700", rotate=0, animate_rotation=ft.Animation(250, ft.AnimationCurve.EASE_IN_OUT))
                    status_text = ft.Text("Shuffling...", color="white", size=16)

                    ajo_draw_overlay_container.content = ft.Container(
                        content=ft.Column(
                            [
                                ft.Text("🎉 Ajo Draw", color="#FFD700", weight=ft.FontWeight.BOLD, size=24),
                                dice_icon,
                                status_text,
                                result_col,
                            ],
                        ...
                        padding=30, alignment=ft.Alignment(0, 0), expand=True,
                    )
                    ...
                    for _ in range(4):
                        ...
                        page.update()
                        await asyncio.sleep(0.25)
                    status_text.value = "Revealing order..."
                    page.update()

                    for i, name in enumerate(members):
                        y_str, m_str = round_months[i].split("-")
                        month_label = f"{MONTH_NAMES[int(m_str) - 1]} {y_str}"
                        ball = ft.Container(
                            content=ft.Column(
                                [
                                    ft.Text(f"#{i + 1}", color="#FFD700", weight=ft.FontWeight.BOLD, size=14),
                                    ft.Text(name, color="white", weight=ft.FontWeight.BOLD, size=20),
                                    ft.Text(month_label, color="#2E8B57", weight=ft.FontWeight.BOLD, size=13),
                                ],
                            ...
                            scale=0,
                            animate_scale=ft.Animation(450, ft.AnimationCurve.BOUNCE_OUT),
                        )
                        result_col.controls.append(ball)
                        page.update()
                ...
                ajo_content_area.content = ft.Container(
                    content=ft.Column(
                        [
                            ...
                            members_col,
                            ft.Row([new_member_field, ft.IconButton(icon=ft.Icons.ADD_CIRCLE_ROUNDED, icon_color="#2E8B57", icon_size=28, on_click=add_member)]),
                            ft.Divider(height=1, color="#4B0082"),
                            ft.ElevatedButton(
                    ...
                    ft.Container(
                        content=ft.Row(
                            [
                                ...
                                ft.Text(m, color="white", size=13, weight=ft.FontWeight.BOLD),
                            ],
                            spacing=10,
                        ),
                        on_click=make_toggle_paid(m),
            ...
            ajo_content_area.content = ft.Container(
                content=ft.Column(
                    [
                        ...
                        ft.ElevatedButton("Advance to Next Round", on_click=advance_round, style=ft.ButtonStyle(bgcolor="#2E8B57", color="white")),
                    ],
                    spacing=10,
                ),
                padding=15, border_radius=15,
        ...
        async def create_ajo_group_dialog(e=None):
            ...
            async def create_it(e):
                ...
                if not gname:
                    ...
                    return
                groups = await load_ajo_groups()
                new_group = {"id": new_ajo_id(), "name": gname, "members": [], "contribution_amount": 0, "order": [], "current_round": 1, "payments": {}, "history": []}
            ...
            dlg = ft.AlertDialog(
                ...
                content=ft.Container(content=ft.Column([name_field, error_text], tight=True, spacing=10), width=280),
                actions=[
                    ft.TextButton("Cancel", on_click=lambda e: page.pop_dialog(), style=ft.ButtonStyle(color=PURPLE_TEXT)),
        ...
        async def open_ajo_switcher(e):
            ...
            async def export_ajo_group(e):
                ...
                if not current_active_group:
                    return
                export_bytes = json.dumps(current_active_group, indent=2).encode("utf-8")
                file_path = await pdf_file_picker.save_file(
