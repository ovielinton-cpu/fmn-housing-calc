    ...
    async def build_main_app():
        ...
        async def refresh_ajo_ui():
            ...
            if current_round > len(order):
                ...
                async def do_start_new_cycle(e):
                    page.pop_dialog()
                    active_group["order"] = []
                ...
                async def do_delete_group(e):
                    page.pop_dialog()
                    fresh_groups = await load_ajo_groups()
                    fresh_groups = [g for g in fresh_groups if g["id"] != active_group["id"]]
                    await save_ajo_groups(fresh_groups)
                    await save_active_ajo_id(fresh_groups[0]["id"] if fresh_groups else None)
                    await refresh_ajo_ui()

                async def save_history(e):
                    active_group.setdefault("past_cycles", []).append(list(active_group.get("history", [])))
                    await save_ajo_groups(groups)

                    choice_dlg = ft.AlertDialog(
                        modal=True, bgcolor="#FFFFFF",
                        title=ft.Text("History Saved ✅", color=PURPLE_TEXT, weight=ft.FontWeight.BOLD),
                        content=ft.Text("What would you like to do next?", color=DARK_TEXT, size=13),
                        actions=[
                            ft.TextButton("Delete This Group", on_click=do_delete_group, style=ft.ButtonStyle(color="#C62828")),
                            ft.TextButton("Start New Cycle (Same Members)", on_click=do_start_new_cycle, style=ft.ButtonStyle(color="#2E8B57")),
                        ],
                        actions_alignment=ft.MainAxisAlignment.END,
                    )
                ...
                ajo_content_area.content = ft.Container(
                    content=ft.Column(
                        [
                            ...
                            ft.Divider(height=1, color="#4B0082"),
                            ft.Text(
                                "Tap 'Save History' to archive this cycle's results. You'll then choose to start a new cycle with the same members, or delete this group. Saved history can always be found later via the group switcher → 'Past Cycles'.",
