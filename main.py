...
async def build_ui(page: ft.Page):
    ...
    async def build_main_app():
        ...
        mt_date_button_text = ft.Text("Tap to pick a date (optional — defaults to today)", color=PURPLE_TEXT, size=13, weight=ft.FontWeight.BOLD)
        ...
        mt_date_button = ft.OutlinedButton(
            content=mt_date_button_text,
            on_click=lambda e: page.show_dialog(mt_date_picker),
            style=ft.ButtonStyle(side=ft.BorderSide(1, PURPLE_TEXT)),
        )
        ...
        def open_edit_dialog(actual_idx, entry):
            ...
            edit_date_button = ft.OutlinedButton(
                content=edit_date_button_text,
        ...
        for i, label in enumerate(tab_labels):
            tab_buttons.append(
                ft.ElevatedButton(
                    content=ft.Row(
                        [ft.Icon(tab_icons[i], size=16, color="#4B0082" if i == 0 else "white"), ft.Text(label, size=12, weight=ft.FontWeight.BOLD, color="#4B0082" if i == 0 else "white")],
